...
localPostgres <- function(client = Sys.getenv("TEST_PG_DRIVER", "RPostgres")) {
  ...
  if (client == "RPostgres") {
    ...
    DBI::dbExecute(con_duck, "CALL postgres_execute('pg_db', 'CREATE SCHEMA IF NOT EXISTS public');")
    DBI::dbExecute(con_duck, "CALL postgres_execute('pg_db', 'CREATE SCHEMA IF NOT EXISTS results');")
    con_duck
